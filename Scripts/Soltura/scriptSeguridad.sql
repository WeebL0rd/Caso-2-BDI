...
-- 1. CREACI�N DE USUARIOS Y CONTROL DE ACCESO
...
-- Denegar acceso completo a la base de datos (revocar permisos de conexi�n)
DENY CONNECT TO usuario;


-- PRUEBA DE CONEXI�N RESTRINGIDA
USE solturaDB;
EXECUTE AS USER = 'usuario';

SELECT firstName FROM solturaDB.sol_users; -- Cualquier consulta simple

-- Esto tambi�n verifica si el usuario puede conectarse a la DB
...
-- PRUEBAS DE PERMISO DE SELECT A UN USUARIO ESPEC�FICO
-- Ejemplo con la tabla de pagos, que contiene la informaci�n de los pagos realizados
-- usuario no podr� acceder
EXECUTE AS USER = 'usuario';
SELECT * FROM solturaDB.sol_payments; -- Error
REVERT;

-- empleado no podr� acceder
...
-- empleado en rol SoloLectura (antes ten�a restringido el acceso a la tabla, pero ahora accede por el rol)
EXECUTE AS USER = 'empleado';
SELECT * FROM solturaDB.sol_payments; -- OK
REVERT;
...
-- el usuario no puede utilizar el SP si no se le da permiso expl�citamete
